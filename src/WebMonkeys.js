load(this, function(exports) {
  function WebMonkeys(opt) {
    ...
    function buildShader(vertexSrc, fragmentSrc) {
      function compile(type, shaderSource) {
        ...
        if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
          var errorMsg =
            "WebMonkeys had the following error from WebGL: " +
            gl.getShaderInfoLog(shader);
          if (errorMsg.indexOf("syntax error") !== -1)
            errorMsg +=
              "This could be fixed by adding extra `;` before setters.";
          throw errorMsg;
        }
        return shader;
      }
    ...
    function fitTextureSide(elements) {
      return Math.pow(
        2,
        Math.ceil(Math.log(Math.sqrt(elements)) / Math.log(2))
      );
    }
    ...
    function get(name) {
      ...
      if (!targetArray) {
        ...
        for (var i = 0, l = array.length; i < l; ++i) {
          ...
          var m =
            1 +
            pixels[i * 4 + 0] / 256 / 256 / 256 +
            pixels[i * 4 + 1] / 256 / 256 +
            pixels[i * 4 + 2] / 256;
    ...
    function set(name, lengthOrArray) {
      if (typeof lengthOrArray === "number") {
        ...
        if (lengthOrArray instanceof Array) {
          ...
          for (var i = 0, l = lengthOrArray.length; i < l; ++i) {
            ...
            array[i * 4 + 0] =
              Math.floor(fract((m - 1) * 256 * 256) * 256) || 0;
      ...
      if (!arrayByName[name]) {
        ...
        arrayByName[name] = {
          name: name,
          uint32Array:
            lengthOrArray instanceof Uint32Array ? lengthOrArray : null,
    ...
    function buildTask(task) {
      ...
      var setter;
      while (
        (setter = parseSetterStatement(
          taskStatements[taskStatements.length - 1] + ";"
        ))
      ) {
        setters.push(setter);
        taskStatements.pop();
      ...
      for (var i = 0; i < allocResults * 2; ++i)
        writeToTexture +=
          "  if (idx == " + i + ") gl_FragColor = results[" + i + "];\n";
    ...
    function work(monkeyCount, taskSource) {
      ...
      allocVertexIndices(
        Math.max(
          monkeyCount,
          monkeyCount * resultSquareSide * resultSquareSide / 2
        )
      );
      ...
      gl.bindFramebuffer(gl.FRAMEBUFFER, framebuffer);
      gl.uniform1f(
        gl.getUniformLocation(task.shader, "resultGridSide"),
        resultGridSide
      );
      gl.uniform1f(
        gl.getUniformLocation(task.shader, "resultSquareSide"),
        resultSquareSide
      );
      gl.uniform1f(
        gl.getUniformLocation(task.shader, "resultTextureSide"),
        resultTextureSide
      );
      gl.vertexAttribPointer(
        gl.getAttribLocation(task.shader, "resultIndex"),
        1,
        gl.FLOAT,
        false,
        0,
      ...
      for (var i = 0, l = arrays.length; i < l; ++i) {
        ...
        gl.uniform1i(
          gl.getUniformLocation(task.shader, arrays[i].textureName),
          i
        );
      }
      ...
      gl.bindTexture(gl.TEXTURE_2D, resultTexture);
      gl.uniform1i(
        gl.getUniformLocation(writer, "resultTexture"),
        resultTexture
      );
      gl.uniform1f(
        gl.getUniformLocation(writer, "resultGridSide"),
        resultGridSide
      );
      gl.uniform1f(
        gl.getUniformLocation(writer, "resultSquareSide"),
        resultSquareSide
      );
      gl.uniform1f(
        gl.getUniformLocation(writer, "resultTextureSide"),
        resultTextureSide
      );
      gl.uniform1f(
        gl.getUniformLocation(writer, "targetTextureSide"),
        resultArray.textureSide
      );
      gl.vertexAttribPointer(
        gl.getAttribLocation(writer, "resultIndex"),
        1,
        gl.FLOAT,
        false,
        0,
